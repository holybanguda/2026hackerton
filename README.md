...
### 📱 v1.1.0 (2026-08-12)
> **모바일 프론트엔드 React UI 100% 구축 & Universal Naver Place 100% 실시간 라이브 파서 연동 완료! 🌐📷**
1. **🎨 16개 피그마(Figma) 디자인 1:1 정밀 반영 프론트엔드 모바일 앱 구축 (`front/`):**
   - Coral Orange 브랜드 디자인 시스템(`pick nu`), Pretendard 폰트 및 모바일 프레임 적용.
   - 7단계 전체 맞춤 추천 인디케이터 플로우 (인원수 ➔ 1인당 예산 슬라이더 ➔ 모임 분위기 ➔ 대식가/다이어터/맵기 ➔ 알레르기 제약 ➔ AI 4단계 순차 체크리스트 팝 애니메이션 ➔ 4대 레이어 추천 조합 결과).
   - 50분 경과 2차 안주/디저트 재추천 시뮬레이터 및 이전 추천 히스토리 탭 완벽 연동.

2. **🌐 Universal Naver Place 100% 실시간 라이브 파서 연동 (`GET /ai/parse-url`):**
   - 네이버지도 PC/모바일의 복잡한 Client-Side SPA 및 GraphQL/Apollo State 구조를 100% 우회 수집하는 범용 파서 구축.
   - 임의의 네이버지도 URL(`m.place.naver.com` / `map.naver.com/.../place/ID`) 입력 시 1초 만에 식당 상호명, 18~30여 개 전체 실시간 라이브 메뉴 및 가격 100% 자동 추출.
   - 라이브 수집 모션 애니메이션 모달 오버레이 및 CORS 무장애 미들웨어 설정 완료.
